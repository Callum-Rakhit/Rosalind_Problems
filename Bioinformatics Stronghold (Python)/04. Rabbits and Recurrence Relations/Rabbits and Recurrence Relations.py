...
# finite sequence and the infinite sequence of odd numbers (1,3,5,7,9). We use
# the notation an to represent the n-th term of a sequence.
#
...
# of offspring in any month is equal to the number of rabbits that were alive two
...
with open(sys.argv[1], 'r') as input_string:
    string = input_string.readlines()


months = int(string[0])
litter_size = int(string[1])


def rabbits(mature, immature, litter_size, months):
    number = None
    if months == 0:
        number = 0
    if months == 1:
        ...
        number = rabbits(immature, mature * litter_size + immature, litter_size, months - 1)

    return number


print rabbits(0, 1, litter_size, months)
